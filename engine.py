...
class Game:
    ...
    def smart_hunting_ai(self):
        ...
        # Track the potential direction of the ship from the hits
        possible_targets = []
        for hit in hits:
            row = hit // 10
            col = hit % 10
            # Check surrounding squares and add to possible targets
            for r, c in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                new_row = row + r
                new_col = col + c
                if 0 <= new_row < 10 and 0 <= new_col < 10:
                    target_square = new_row * 10 + new_col
                    if search[target_square] == "U":
                        possible_targets.append(target_square)

        if possible_targets:
            # Prioritize the targets based on proximity to hits
            self.make_move(random.choice(possible_targets))
        else:
            # If no clear targets, fallback to basic AI or random
            self.basic_ai()

    def converging_search_ai(self):
        search = self.player1.search if self.player1_turn else self.player2.search
        unknown = [i for i, square in enumerate(search) if square == "U"]
        hits = [i for i, square in enumerate(search) if square == "H"]
        ...
        # Find the center of hits and focus search around it
        closest_hit = hits[-1]  # Start from the latest hit
        row = closest_hit // 10
        col = closest_hit % 10
        possible_moves = []

        # Search closer squares based on the last hit
        for r, c in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            new_row = row + r
            new_col = col + c
            if 0 <= new_row < 10 and 0 <= new_col < 10:
